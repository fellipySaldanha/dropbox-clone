class DropBoxController {
    ...
    removeFolderTask(reference, name){
        return new Promise((resolve, reject) => {
            ...
            let folderRef = this.getFirebaseReference(reference + '/' + name);
            ...
            this.uploadTask(event.target.files)
                .then(responses => {
                    ...
                    responses.forEach(response => {                        
                        ...
                        this.getFirebaseReference().push().set(parseFireStorage);
                    });
                    this.uploadComplete();
                })
                .catch(error => {
    ...
    uploadProgress(event, file) {
        ...
        this.timeLeftEl.innerHTML = this.formatTimeDisplay(timeleft);
    ...
    openFolder() {
        ...
        if (this.lastFolder) {
            this.getFirebaseReference(this.lastFolder)
                .off('value'); 
        }
    ...
    renderNav() {
        ...
        this.currentFolder.forEach(folderName => {

            let span = document.createElement('span');
            count++;
           
            path.push(folderName);           
            if (count == this.currentFolder.length) {
        ...
        li.addEventListener('click', event => {
            ...
            if (event.shiftKey) {               
                ...
                if (firstLi) {                    
                    ...
                    let liSelect = [indexStart, indexEnd].sort();
                  
                    listLi.forEach((element, index) => {
                        if (index >= liSelect[0] && index <= liSelect[1]) {
